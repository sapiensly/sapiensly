...
#[Description('Dry-run a workflow to debug it: every write is simulated (never applied) and side-effect-free steps run for real. Returns the run status and per-step trace.')]
class VerifyWorkflowTool extends SapiensTool
{
    ...
    public function handle(Request $request): Response
    {
        ...
        $workflow = collect($manifest['workflows'] ?? [])
            ->firstWhere('id', $validated['workflow_id']);
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'app_slug' => $schema->string()
                ->description('The slug of the app that owns the workflow.')
                ->required(),
            'workflow_id' => $schema->string()
                ->description('The id of the workflow to verify.')
                ->required(),
            'trigger_payload' => $schema->object()
                ->description('Optional payload to simulate the trigger with.'),
        ];
    }
}
