...
#[Description('Apply a set of RFC 6902 JSON Patch operations to an app\'s manifest. The patch is validated and, if valid, saved as a new (reversible) app version. Read the manifest first to target the right paths. Wherever an id is expected you may write the SLUG instead (\'object_id\':\'refacciones\', \'field_id\':\'sku\', \'role_id\':\'tecnico\', \'page_id\':\'ordenes\') and it is resolved for you — a field against the object its block queries. Real ids still work and win; a name matching nothing is rejected, never guessed. On success returns {applied:true, version_number, changed_paths:[{op, path, from?}]} — array appends ("/-") are resolved to the concrete index they landed at, so you can target follow-up patches without re-reading — plus {warnings} when the patch applied but built something that will do nothing (a button aimed at no page, a list you cannot open from); fix those or say what you could not complete before reporting success. If rejected, returns {applied:false, valid:false, errors:[{path, message, code, expected?, value?}], warnings} — the same structured detail as validate_manifest — so you can fix every error and retry.')]
