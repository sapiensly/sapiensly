...
#[Description("The organization's AI spend over a window — today, this week, this month, or the last 7/30/90 days: total cost, calls and tokens, the own-key vs platform-key split, a per-model breakdown, a per-service breakdown (Chat, Apps, … each with its own per-model split), the same spend grouped the other way round by the named artifact it was made on (an app, a chatbot, a chat, a deck, a knowledge base) with a per-service split under each, a cost series (hourly for today, otherwise daily), and the configured budgets/caps. Spend recorded before artifact tagging shipped appears as one unattributed line rather than being dropped. Owner-only.")]
