...
class SetOrganizationContextTool extends SapiensTool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'geographies' => $schema->array()->items($schema->string())->description('Countries or regions it operates in. Replaces the stored list. Max 10.'),
            ...
            'glossary' => $schema->array()->items($schema->object())->description('Organization-specific vocabulary: array of {term, meaning}. The highest-value field — internal terms whose meaning differs from the common one. Replaces the stored list. Max 20.'),
            'offerings' => $schema->array()->items($schema->object())->description('Main products/services: array of {name, description}. Replaces the stored list. Max 10.'),
            'never' => $schema->array()->items($schema->string())->description('Things an agent must never do, one short sentence each (e.g. "Quote prices"). Replaces the stored list. Max 10.'),
            ...
            'links' => $schema->array()->items($schema->object())->description('Canonical URLs: array of {label, url}, so agents cite real links instead of inventing them. Replaces the stored list. Max 8.'),
