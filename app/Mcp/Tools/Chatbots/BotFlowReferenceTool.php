...
class BotFlowReferenceTool extends SapiensTool
{
    ...
    private function reference(): array
    {
        return [
            ...
            'inline_roster_layers' => "An `agent_handoff` node may carry its roster INLINE as `layers` — `{ triage|knowledge|tools: { enabled: bool, agent_id } }`. This is a second, equivalent way to declare the roster (tools → the `action` role): each enabled layer with an agent_id binds that agent to the role, exactly like a standalone `agent` node. So a flow's roster is the union of its `agent` nodes AND any agent_handoff layers (first-seen wins per role). `mode` ('agent' | 'multi_agent') is editor-only presentation and ignored at runtime. When authoring via update_bot_flow you may use either form: standalone `agent` nodes, or `layers` on the agent_handoff node.",
            'node_types' => [
                [
                    'type' => 'start',
