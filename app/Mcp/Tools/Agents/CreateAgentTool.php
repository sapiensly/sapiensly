...
class CreateAgentTool extends SapiensTool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'keywords' => $schema->array()->items($schema->string())->description('Optional keywords for search/categorization.'),
            ...
            'knowledge_base_ids' => $schema->array()->items($schema->string())->description('Knowledge base ids to attach (from list_knowledge_bases).'),
            'tool_ids' => $schema->array()->items($schema->string())->description('Tool ids to attach (from list_tools).'),
