...
- Shape: `questions` (a query — FILTER it to one questionnaire and SORT it), `label_field_id`, `type_field_id` + `type_map` (the app's own vocabulary → an input kind, e.g. {"escala_1_5":"rating","si_no":"boolean"}), and `answers` {object_id, question_field_id, value_field_ids: ONE COLUMN PER KIND, parent_field_id?}. A kind with no column is dropped silently at file time, so map every kind the questions object can hold.
