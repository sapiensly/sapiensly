...
- A "connected object" reads LIVE from a REST OR MCP connection instead of the internal records store:
  ...
  - MCP: call `sample_mcp_tool` (integration_id, then the list tool) to see the shape, then propose an object whose `source` is {type:"connected", integration_id, operations:{list:{mcp_tool:"<tool>", arguments?:{…}, collection_path?:"…"}}, id_path, field_map:[{field_id, external_path}]} — NO method/path for MCP; the tool's JSON rows feed field_map/id_path. Read uses the integration's own credentials (org-level), so the connection needs a service/static token; a per-user-only OAuth connection can't back a shared live dashboard.
