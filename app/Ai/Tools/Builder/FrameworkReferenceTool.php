...
3. COMPOSE BESPOKE: author each section as an `html` block (semantic markup with YOUR OWN class/id) and put ALL styling in settings.custom_css targeting those classes (landing budget: up to 60,000 chars). SCOPING RULE: custom_css is auto-wrapped in the app surface via CSS nesting — put your CSS VARIABLES and page background on your OWN wrapper class (e.g. `.lp{--brand:#…;background:…}`) that wraps every html section, NOT on `:root`/`body`/`html` (the compiler rewrites those to the surface as a safety net, but your own class is the reliable pattern). Declare @keyframes normally at top level — the compiler hoists them. COPY DIALECT: write in the AUDIENCE's variety of the language — es-MX tuteo ("reserva tu clase") for Mexico; voseo only for a rioplatense audience. Aim for: a real display type scale (clamp up to ~3-4.5rem, tight tracking), a committed palette (a chosen neutral + a confident accent), asymmetry and tension (NOT everything centered), depth (gradients/glow/grain). Inline `style`, <script>, <style> and event handlers are STRIPPED on save — styling lives in custom_css only.
