...
- CHART SHAPE — get it right the FIRST time (these are the common misfires): a `chart` takes `chart_type`, `data_source` ({object_id, filter?, sort?, limit?} — NOTHING else; the breakdown does NOT go here), `aggregation`, and — for the breakdown — `group_by_field_id` ON THE CHART (not in data_source). Charts aggregate with count|sum|avg|min|max ONLY: for sum/avg/min/max also set `y_field_id` (the numeric field); count needs none. median/p90/p95 are NOT chart aggregations — put those in a `stat` / `metric_grid` KPI instead. `data_source.sort` is an ARRAY: [{field_id, direction:"asc"|"desc"}]. A date/datetime X → set `bucket` (see TIME-AXIS below). ONE chart reads ONE object (`data_source.object_id`): `series_field_id` and `series` split/overlay WITHIN that object's rows (the values of one categorical field, or several measures of the SAME rows) — they cannot merge two SEPARATE objects into a single chart. So to compare two connected objects that share a schema (e.g. OTD by sale-date vs OTD by delivery-date, which acquire as two objects), you have two honest options: TWO charts side by side (each reads its object), or acquire/re-model the source as ONE object carrying a category column (e.g. `reference_date` with rows for both) and split it with `series_field_id`. Do NOT claim one chart shows both series unless a `series_field_id`/`series` over one object actually produces them.
