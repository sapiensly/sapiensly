...
class ListAvailableFieldTypesTool implements Tool
{
    ...
    public function handle(Request $request): string
    {
        $catalog = [
            ...
            ['type' => 'file', 'props' => 'max_size_mb (1-100, default 10), mime_types? (array of MIME patterns like ["image/*", "application/pdf"]; omit to allow anything), capture? ("camera"). Stored as object {file_id, original_name, mime, size_bytes, url}. Uploads go through POST /r/{slug}/uploads, served via GET /r/{slug}/files/{file_id} with tenant auth. Good for contracts, photos, ID scans, attachments. Filters on file fields: only is_null / is_not_null in MVP. SET capture:"camera" when the answer is something the person is standing in front of — damage reports, proof of delivery, meter readings, site inspections: it opens the camera on a phone and falls back to the file picker on a desktop, so it is always safe to use.'],
