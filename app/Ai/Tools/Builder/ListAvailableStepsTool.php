...
class ListAvailableStepsTool implements Tool
{
    ...
    public function handle(Request $request): string
    {
        $catalog = [
            ...
            [
                'type' => 'notify.send',
                'props' => 'to (array of: an email address, "user:<id>", "role:<app role slug>", or "owner" — expressions allowed), subject, body, channel? (email|in_app|push, default email), link?',
                'output' => '{sent, channel, recipients, unresolved, throttled, failed}',
                'note' => 'THE way to tell a person something happened — use it instead of a connector.call for plain email. `to` entries are expression-resolved first, so "{{trigger.record.data.email}}" reaches whoever submitted the record, and "role:admin" reaches everyone granted that role in the Access panel. channel=in_app raises it in the app\'s notification bell instead of sending mail. channel=push reaches the person\'s phone while the app is CLOSED — the one channel that finds a technician who has been handed a job while driving; it is addressed to a PERSON and delivered to every device they allowed notifications on, so it needs a "user:"/"role:"/"owner" reference rather than a bare email address, and it reports plainly when somebody has no device registered (which is the normal state until they say yes). Use it for what is urgent and useless later — an assignment, an escalation, a delivery at the gate — and email for what somebody reads at a desk. The step NEVER fails the run over an unreachable recipient: it reports `unresolved` (nothing matched), `throttled` (past the organization\'s hourly ceiling — 20 recipients per step, 200 sends per hour) and `failed`, so read the output and tell the user what actually went out. During verify_workflow nothing is sent (simulated: true).',
            ],
            [
                'type' => 'record.create',
