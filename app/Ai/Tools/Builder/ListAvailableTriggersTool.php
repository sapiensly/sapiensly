...
class ListAvailableTriggersTool implements Tool
{
    ...
    public function handle(Request $request): string
    {
        $catalog = [
            ...
            ['type' => 'location.arrived', 'props' => 'object_id (required), filter? — fires when somebody RECORDING THEIR ROUTE comes within the geofence drawn around that record\'s `geo` field. Payload: {record: {...}, location: {lat, lng, distance_m}}. Requires settings.tracking.enabled plus a `tracking` block on the page the person is on; nothing fires for somebody who never started a trail. Never fires for the state a trail STARTED in — arriving at a site you were already standing at is not an arrival. THIS is the reason to turn tracking on: use it to stamp a visit, tell a customer the technician is outside, or advance a route step.'],
            ['type' => 'location.departed', 'props' => 'object_id (required), filter? — the same fence, on the way out. Judged against a slightly wider radius than arrival so a phone sitting still at the edge does not fire arrive/depart all afternoon. Use it to close a visit and compute how long it took.'],
            ...
            ['type' => 'conversation.escalated', 'props' => 'channel_id (required, a WhatsApp/widget channel in your org) — fires when a bot hands a conversation to a person (a human_handoff node with notify on). This is how the team finds out. Payload: {channel: {id, type, name}, chatbot: {id, name}, conversation_id, reason, mode, contact: {id, name, email, identifier}}'],
