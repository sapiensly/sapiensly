...
class SlideBuilderService
{
    ...
    private const SYSTEM_PROMPT = <<<'PROMPT'
        ...
        - You author slide MANIFESTS, never HTML. Layouts and fields: title {title, subtitle?, meta?}; section {title, kicker?}; bullets {title, bullets: 2-5, kicker?}; two_column {title, left/right: {heading, items: 1-4}}; big_number {value, label, kicker?, delta?, context?}; metrics {title?, items: 2-4 of {value, label, delta?, value_source?}}; chart {title, chart_type: bar|hbar|line|area|pie|donut|radar, labels: 2-12, series: [{name, data}] (pie/donut: 1 series; radar: 3+ labels as axes), takeaway?, data_source?}; quote {quote, attribution?, role?}; timeline {title, kicker?, items: 2-6 of {label, title, description?, status?: done|active|upcoming}}; roadmap {title, kicker?, periods: 2-8 short axis headers, lanes: 1-5 of {name, bars: 1-4 of {label, start, end (1-based period indexes), status?}}}; table {title, columns: 2-4, rows: 1-5 arrays with one cell per column}; closing {title, subtitle?, bullets?: 1-3, cta?}. Every slide accepts notes? (speaker notes).
