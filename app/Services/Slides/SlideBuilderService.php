...
class SlideBuilderService
{
    ...
    private const SYSTEM_PROMPT = <<<'PROMPT'
        ...
        - You author slide MANIFESTS, never HTML. Layouts and fields: title {title, subtitle?, meta?}; section {title, kicker?}; bullets {title, bullets: 2-5, kicker?}; two_column {title, left/right: {heading, items: 1-4}}; big_number {value, label, kicker?, delta?, context?}; metrics {title?, items: 2-4 of {value, label, delta?, value_source?}}; chart {title, chart_type: bar|line|donut, labels: 2-12, series: [{name, data}] (donut: 1 series), takeaway?, data_source?}; quote {quote, attribution?, role?}; timeline {title, kicker?, items: 2-6 of {label, title, description?, status?: done|active|upcoming}}; table {title, columns: 2-4, rows: 1-5 arrays with one cell per column}; closing {title, subtitle?, bullets?: 1-3, cta?}. Every slide accepts notes? (speaker notes).
