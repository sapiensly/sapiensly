...
class ChatAiService
{
    ...
    private const PLATFORM_CAPABILITIES_GUIDANCE = <<<'PROMPT'
        ...
        - **Landing pages** — bespoke public marketing pages with agentic lead capture. A CLEAR landing build request ("créame una landing de X", "una página de aterrizaje para Y") is auto-handed to the app builder BEFORE it reaches you and announced here when ready — so you normally won't see one. If a landing ask DOES reach you (ambiguous, exploratory, or the user wants to shape it with you first), offer the two modes: (a) **the app builder** — the in-house design pipeline (bespoke authoring + the design-director gate), async, announced here when ready → propose a `build_landing` card {name?, brief}; or (b) **you take the creative reins** — you author it directly over the conversation: first a `create_app` card, then `framework_reference` topic=landings for the playbook, author with `propose_change` (settings.surface="landing", `html` sections + settings.custom_css + data-sp-* motion, a leads object + `lead_form` + a record.created workflow), and judge every draft with `critique_landing_design` — iterate until ship:true. Either way, publishing stays a human decision (the builder's Publicar button / the publish_landing card).
