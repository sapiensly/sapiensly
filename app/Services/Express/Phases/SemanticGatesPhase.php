...
class SemanticGatesPhase implements ExpressPhase
{
    ...
    {
        return $context->tr('Tuning the spec and writing the insights…');
    }
...
TXT,
            ...
            fn ($schema) => [
                ...
                'insights' => $schema->array()->items($schema->object())->description('[{variant, title, body}] mismo orden y cantidad.'),
            ],
            fn () => [
                'title' => (string) ($spec['title'] ?? 'Dashboard'),
                'purpose' => '',
