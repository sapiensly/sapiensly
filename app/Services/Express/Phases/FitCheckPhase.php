...
TXT,
                ...
                fn ($schema) => [
                    ...
                    'pieces' => $schema->array()->items($schema->object())->description('OBLIGATORIO: una entrada por CADA pieza pedida: [{asked, tool: nombre exacto del tool que la responde o null, proxy: métrica sustituta honesta si tool es null y existe una}].'),
                    'substitutions' => $schema->array()->items($schema->object())->description('[{asked, using, reason}] proxies honestos.'),
                    'unanswerable' => $schema->array()->items($schema->object())->description('[{asked, reason}] piezas pedidas sin respuesta en estos datos.'),
