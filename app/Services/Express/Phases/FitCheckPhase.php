...
TXT,
                ...
                fn ($schema) => [
                    'tools' => $schema->array()->items($schema->string())->description('Nombres exactos de tools del catálogo a leer (1-4).'),
                    'pieces' => $schema->array()->items($schema->object(['asked' => $schema->string()]))->description('OBLIGATORIO: una entrada por CADA pieza pedida: [{asked, tool: nombre exacto del tool que la responde o null, proxy: métrica sustituta honesta si tool es null y existe una}].'),
                    'substitutions' => $schema->array()->items($schema->object(['asked' => $schema->string(), 'using' => $schema->string(), 'reason' => $schema->string()]))->description('[{asked, using, reason}] proxies honestos.'),
                    'unanswerable' => $schema->array()->items($schema->object(['asked' => $schema->string(), 'reason' => $schema->string()]))->description('[{asked, reason}] piezas pedidas sin respuesta en estos datos.'),
                    'core_unanswerable' => $schema->boolean(),
                    'alternatives' => $schema->array()->items($schema->string())->description('Si core_unanswerable: 1-2 dashboards que SÍ se pueden construir.'),
                ],
