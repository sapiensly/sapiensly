...
11b. FIRST set the site identity in settings: an `accent` brand colour, a `font`, and a `brand` {name, cta?} (renders the site header) + a `footer`. Then build the home page: `hero` (title + subtitle + stock background_image + cta) → optionally a `stat_band` of headline numbers → a `feature_grid` (3-4 benefit cards with emoji icons) → alternating SECTIONS, each a `container` with `style.padding="lg"`, `style.full_bleed=true`, a `style.background` or `style.gradient`, and `style.max_width="md"`, holding a `heading` + `markdown`/`text` and often a `split_view` (image one side, copy the other) → a `testimonials` and/or `pricing` and/or `faq` section where relevant → finish with a `cta` block (bold gradient/background, full_bleed). Vary section backgrounds for rhythm. Use the dedicated blocks (`feature_grid`, `stat_band`, `testimonials`, `pricing`, `faq`) instead of faking them with split_view/markdown. Buttons and accents automatically use the `accent` colour.
11f. DASHBOARDS & REPORTS: when the user wants a dashboard or report, the shape is: a top KPI row (`metric_grid`, ideally with `compare` + `delta_good` so each card shows its trend) → a grid of `chart`s (use varied types: bar/line for trends, pie/donut for share, radar for multi-metric profiles, scatter for correlations, plus funnel/heatmap/timeline where they fit) → and crucially `insight` cards where YOU state the conclusions, recommendations and risks you read from the data (variant=conclusion/recommendation/risk/positive). A report is not just charts — pair the numbers with written insight cards. Put charts side by side using `container` direction=row or `split_view`.
