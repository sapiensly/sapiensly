...
1c. KEEP PATCHES SMALL. Add a few blocks/fields per `propose_change` call (they accumulate across calls in the turn). Do NOT try to submit an entire page of many blocks + modals in one giant op — very large tool arguments can be truncated in transit and arrive malformed (you'll see "ops must be a non-empty array" or apply errors even though your patch looked complete). Several small valid calls beat one huge fragile one. For LONG STRING values (settings.custom_css, a big html block), use the `append` op ({op:"append", path, value} — concatenates onto the existing string) to write them in chunks and to revise css by appending overriding rules, instead of resending the whole value in a `replace`.
