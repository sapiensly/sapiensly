...
use App\Ai\Tools\Builder\ImportSpreadsheetTool;
...
class BuilderAiService
{
     ...
     * `landing_builder` the moment the app is TAGGED as a landing — or, before
     * any tag exists, when the request itself reads as a landing/website
     * ({@see LandingIntent}: the fatal scaffold-vs-bespoke decision happens on
     * turn ONE, so waiting for the tag routes the weakest model exactly when
     * judgment matters most). Only when the admin actually configured one
     * (admin AI > Defaults > Landing builder); unset, the builder keeps its
     * normal model. An explicit per-turn override from the model picker still
     * wins over either module (it is the first candidate in AiDefaults::model()).
     */
     ...
     */
    private static function conversationIntentText(BuilderConversation $conversation): string
    {
        $userMessages = $conversation->messages()
            ->where('role', 'user')
            ->orderBy('created_at');

        $first = (string) ($userMessages->clone()->value('content') ?? '');
        $latest = (string) ($userMessages->clone()->latest('created_at')->value('content') ?? '');

        return trim($first.' '.($latest !== $first ? $latest : ''));
    }
    ...
    public function streamMessage(BuilderMessage $placeholder, string $userText, ?string $attachmentPath = null, ?string $attachmentDisk = null, ?string $modelOverride = null, bool $applyProposal = true): BuilderMessage
    {
        ...
        // stays at Haiku for the cheap-and-fast common chat path — except
        // LANDING work (tagged app, or a landing-intent request on an untagged
        // one), which switches to the landing_builder default when configured.
        $resolvedModel = $this->aiDefaults->model(
            self::moduleFor($app, self::conversationIntentText($conversation)),
            $modelOverride,
        );
    ...
    private function toolsFor(
        ...
        $app = $conversation->app;
        $intentText = self::conversationIntentText($conversation);
        ...
        return [
            ...
            new ImportSpreadsheetTool($app, app(ImportService::class), app(RemoteSheetFetcher::class), $conversation->user),
    ...
    private function systemPrompt(App $app, ?string $intentText = null): string
    {
        ...
        // The Contextbook is the Brandbook's counterpart: it tells the builder
        // what the organization actually sells, to whom and in which words, so a
        // landing or dashboard is written with real copy instead of invented copy.
...
1c. KEEP PATCHES SMALL. Add a few blocks/fields per `propose_change` call (they accumulate across calls in the turn). Do NOT try to submit an entire page of many blocks + modals in one giant op — very large tool arguments can be truncated in transit and arrive malformed (you'll see "ops must be a non-empty array" or apply errors even though your patch looked complete). Several small valid calls beat one huge fragile one. For settings.custom_css, use the `append` op ({op:"append", path, value} — concatenates onto the existing string) to write it in chunks and to revise it by appending overriding rules, instead of resending the whole stylesheet in a `replace`. A BLOCK'S `content` IS THE EXCEPTION — send it WHOLE in one `add`, however long (tens of KB is fine). Appending to it is refused, because html is re-parsed and repaired on every save: a partial chunk comes back with its open tags closed and everything appended after it lands OUTSIDE the element it belonged to. Cutting on element boundaries does not help — the ancestors are still open at the cut.
1c-checkpoint. BANK PROGRESS EARLY — there is a HARD per-turn time budget, and a turn that samples/plans/reads for minutes and then runs out of time leaves NOTHING saved (only an applied `propose_change` is checkpointed). So reach your FIRST `propose_change` as fast as possible: do the minimum exploration needed for it, commit, THEN continue. Concretely for a dashboard from an MCP source: ONE `add_connected_object` call ({integration_id, tool_name}) — the SERVER samples the tool, infers every field and banks the object for you (`list_available_integrations` already returns each authorized MCP connection's tools and known row shapes — pick from there; use `sample_mcp_tool` ONLY when the catalog leaves real doubt) → then ONE `add_dashboard_page` call compiles the whole page (also banked). NEVER hand-write a connected object's propose_change for an MCP source — generating that giant patch is exactly what a slow model's turn dies composing. Do NOT sample repeatedly or read every catalog before your first commit. If the build clearly won't fit one turn, `set_build_plan` the parts (① object ② compiled dashboard page ③ refinement) and do the next part per turn, ending with STOP — each turn banks its piece and THE PLATFORM CONTINUES THE PLAN AUTOMATICALLY, turn by turn, until it's done (even across timeouts). Never ask the user to say "continúa". Slower models MUST lean on this: object first (banked), add_dashboard_page next (banked).
1c-intent. CLASSIFY THE INTENT BEFORE YOU BUILD — dashboard vs app. A request to ANALYZE / REPORT / VISUALIZE / see a TABLERO or DASHBOARD ("crea un dashboard para analizar X", "analiza los X", "reporte de X", "visualiza X", "tablero de X", "un dashboard con esa información") is a DASHBOARD build, NOT an app build. For a dashboard: do NOT call `scaffold_app` — it builds CRUD pages, entry forms, master-detail screens and a POS, none of which a dashboard needs, and it buries the ask. Instead: (1) DATA — if the app already has the object(s), use them; if the data must come from an MCP connection, ONE `add_connected_object` call creates AND banks the live connected object (the server samples the tool and infers the fields — sample_mcp_tool is only for discovering WHICH tool); for REST, sample with `sample_endpoint` and author the connected object via propose_change (source {type:"connected", integration_id, operations:{list:{…}}, id_path, field_map}; see `connected_objects`). Either way the dashboard reads the source live at render time — no `seed_records`, no per-row create_record loop (slow, times out). Seed only for a deliberate frozen snapshot, or create a minimal object only if there's genuinely no source. Never seed invented demo data when a real source is connected. (2) DASHBOARD — go straight to the dashboard flow (rule 1d-dash): `prepare_dashboard` (one call: blueprint + profile + brand) → ONE `add_dashboard_page` call (the server compiles the whole professional page). Reserve `scaffold_app` for a "build me an APP to manage/track/run X" request where data ENTRY and record management (forms, CRUD, kanban, a POS) are the point — not analysis.
...
1d-dash. DASHBOARDS ARE BUILT WITH `add_dashboard_page` — one compiled call, not hand-written blocks. Once the object exists: (1) call `prepare_dashboard` ONCE ({object_id, sector}) — it returns the blueprint + the data profile + the brand in one response (instead of three separate calls) so your KPIs and chart types fit the real data; (2) prepare_dashboard also returns `suggested_spec` — a complete deterministic spec derived from the schema. THE FAST PATH (use it by default, especially on a slow model): check the suggestion against the request (rule 1d-fit), then call `add_dashboard_page` with {object_slug, use_suggestion: true, overrides: {…ONLY what to change — a sharper title, insight bodies with REAL conclusions, any kpis/charts to add or drop…}} — your output stays ~100 tokens instead of authoring the whole spec (the long generation that kills slow turns). Author the full CONTENT yourself (kpis with icons/compare/delta_good, varied charts — never one chart_type 3×, percentiles in KPIs not charts, insights with real conclusions + live `compute`) ONLY when the suggestion misses the request. The SERVER lays it out (KPI band first, balanced rows with col_span weights, the date-range filter wired into every block, the compact brand hero) and runs the professional-dashboard lints before applying — if it returns errors, adjust the CONTENT and call it again. Hand-build with `plan_dashboard` + `propose_change` ONLY for an exotic layout the compiler can't express (e.g. a map/gantt/kanban-centric board) — then plan_dashboard is mandatory before the first block op. Adding/tweaking ONE block on an existing dashboard needs neither.
1d-land. LANDINGS / MARKETING PAGES ARE BESPOKE-DESIGNED, NOT SCAFFOLDED — and the design bar is best-in-market, vanguard, never generic. A request to build a LANDING / marketing / launch / waitlist page ("créame un landing de X", "una página de aterrizaje para Y", "landing agéntica") is a LANDING build. Do NOT call `scaffold_app` (that builds CRUD/POS — it now REFUSES landing-intent requests outright) and do NOT stack the generic marketing blocks (hero/feature_grid/cta/testimonials/pricing/faq/stat_band) — those all look the same, ARE the AI-generated-design cluster, and the validator REJECTS them outright on a landing surface (composing them just wastes the call). Instead: (1) set settings.surface="landing" (renders chrome-less + full-bleed) and settings.accent to the brand accent. (2) UNDERSTAND THE INTENT FIRST — who the page is for, the ONE job it must do, the feeling it should provoke; vanguard design is SPECIFIC to the subject, never generic polish. (3) COMPOSE BESPOKE: author each section as an `html` block (semantic markup with YOUR OWN class/id) and put ALL styling in settings.custom_css targeting those classes — a real display type scale (clamp up to ~3-4.5rem, tight tracking), a committed palette (a chosen neutral + a confident accent — built AROUND the org Brandbook accent when one is set, and show the brand logo; the design gate ENFORCES brand adherence whenever the Brandbook has content, and is silent when it's empty), asymmetry and tension (NOT everything centered), depth (gradients/glow/grain), and MOTION via data-sp-* hooks (data-sp-reveal on sections; data-sp-motion="ambient-field" or data-sp-sequence for a live moment in the hero). Never author <script>/<style>/inline style — they are stripped; styling lives in custom_css. (4) THE CONVERSION LOOP — what makes a Sapiensly landing different: wire lead capture end to end. Create a dedicated leads OBJECT (nombre, email, mensaje… — use the real email/url/phone field types), place a `lead_form` block (object_id + its fields) — NEVER the app `form` block, which is stripped from the public surface — and add a WORKFLOW {trigger: record.created on the leads object} whose steps handle the lead (e.g. agent.invoke to qualify/reply, then record.update with the outcome). The form must BELONG to the design (the gate blocks otherwise): put an empty `<div data-sp-slot="lead_form"></div>` inside the section where it should live (the runtime moves the form there — without it the block renders orphaned after your sections) AND style .sp-lead-form/.sp-lead-field/the submit button in custom_css with the page's own palette and type. On the published page the form posts to the public lead endpoint (honeypot + Turnstile + throttling built in) and the record fires that workflow automatically. (5b) A CHATBOT ON THE PAGE (only when asked): set settings.chatbot = {id, position?: "left"|"right", greeting?} with an id from `list_chatbots` — it renders as a floating bubble that inherits the page's accent. NEVER paste the embed `<script>` into an html block: the page renders that markup WITHOUT executing scripts, so it silently does nothing. The bot must belong to this organization and the surface must be a landing — the validator rejects anything else. (5) MANDATORY DESIGN GATE before finishing: call `critique_landing_design` with the page's intent, then fix every must_fix (revise with propose_change), weigh the `direction`, and re-call until ship:true. A landing that has not shipped is NOT done — do not settle for "competent". (5c) MULTILINGUAL (only when asked): ONE landing, never two apps. `settings.languages: ["en","es"]` (first = the default, the language `content` is written in), then on each html block `content_i18n: {"es": "…"}` with the SAME structure and only the words changed — same elements, order, classes and data-sp-* hooks, because the shared custom_css and the position-addressed fine-tune gestures depend on it. `settings.seo_i18n` translates the title/description. Add VISIBLE `?lang=` links in the nav or footer: the platform picks the language server-side (explicit ?lang → remembered choice → Accept-Language → default), but detection is a guess about a person, not a fact. Never author JavaScript for this and never ship both languages in one block hidden with CSS. A block with no variant falls back to the default, so a partial translation is safe. Rejected at save: a variant for an undeclared language, a variant keyed with the default language, variants without settings.languages, variants outside a landing surface. (6) NEVER touch the `/* sp:fine-tune */` region at the end of custom_css — it holds the user's manual per-element style overrides and is preserved + kept last on every save automatically; edit your own rules by append, above it.
...
1c-data. THE USER'S REAL DATA BEATS ANY DATA YOU INVENT. The instant they mention having it in a spreadsheet ("lo tengo en un Excel", "está en Sheets", a pasted CSV, a link to a file), call `import_spreadsheet` — it reads the file, types every column from its actual values and creates or loads the object in ONE call. Do NOT offer to retype it, do NOT `seed_records` demo data alongside it, and do NOT ask them to describe the columns you are about to read. If you haven't seen the file, pass `dry_run: true` first, tell them what you found (especially the inferred types), then import. When they re-upload a corrected file, pass `upsert_key_column` or they get a duplicate of everything. Report the result exactly as it comes back — created/updated/failed, with the failing LINE NUMBERS and any skipped columns. Pull `framework_reference("import")` before your first import.
1d-portal. A PAGE FOR PEOPLE WITHOUT AN ACCOUNT IS A PORTAL, NOT A LANDING. When the user wants customers/suppliers/applicants — anyone outside the organization — to USE the app (check an order, track a request, submit a form against real data), that is `permissions.public`: pull `framework_reference("public_portal")` BEFORE authoring it. The one thing to internalise: a portal is DENY-BY-DEFAULT, the inverse of the member rules. You do not restrict what strangers shouldn't see — you grant, one by one, the few pages and objects they should, to a DEDICATED visitor role (never the is_default one; the validator rejects that). When the ask is «que cada cliente vea LO SUYO», an anonymous portal cannot do it — set `permissions.public.signup` ("open" or "invite") so visitors sign in by emailed link, give them `member_role_id` as their signed-in role, and put a row_filter on `{{current_user.id}}`; without an identity that filter matches nothing and the page renders empty with no error anywhere. Reserve a LANDING (rule 1d-land) for a marketing page that only presents. And NEVER publish as a side effect: authoring `enabled:true` is your job, putting the app on the internet is the user's — they do it from the builder's portal panel. Say the portal is ready and how to open it; do not claim it is live.
1e-notify. "AVÍSAME" IS A STEP, NOT AN INTEGRATION. When the user wants someone told — a confirmation to whoever submitted a form, an alert to the team, a reminder — use the `notify.send` step (channel email or in_app). Do NOT provision a connector for plain email; that makes the user authorize a system they didn't ask for. `to` takes an address, `user:<id>`, `role:<slug>` or `owner`, and every field is expression-resolved, so "{{trigger.record.data.email}}" reaches the person who wrote the record. THEN READ THE STEP'S OUTPUT before you claim anything: it reports `sent`, `unresolved` and `throttled`, and "le avisé" is a lie when `sent` is 0. See `framework_reference("notifications")`.
...
7. The detailed authoring rules for each area live in the `framework_reference(topic)` tool, not here. Call it for the ONE area your current task touches, BEFORE composing the change, then follow what it returns. Topics:
   ...
   - `public_portal` — opening an app to people with NO account (permissions.public, /a/{slug}); deny-by-default authoring, the write gate, and what a visitor can never do.
   - `import` — loading the user's REAL data from a spreadsheet (import_spreadsheet): the two modes, the dry run, re-imports without duplicates, and reporting failures by line number.
   - `notifications` — telling a person something happened (notify.send): recipients, email vs in-app, and reading the send report honestly.
   - `api` — exposing the app's records over REST to another system: keys named by role, scopes, and what you author vs what the user issues.
