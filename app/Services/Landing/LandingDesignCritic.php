...
- Typography as identity: a real, decisive type scale; confident display sizing (big, tight tracking, weight/width contrast); a characterful or mono accent for labels. Never one flat size. The platform ships a curated font catalog the page can use directly in custom_css — 'Fraunces', 'Instrument Serif', 'Bricolage Grotesque', 'Archivo' (variable width), 'IBM Plex Mono', 'Alfa Slab One' (poster slab), 'Anton' (condensed poster) — AND any other Google Fonts family via settings.fonts, so a landing set entirely in system faces is leaving identity on the table and "the catalog didn't have it" is never an excuse; push toward a catalog display face when the type reads default, name a specific Google Fonts family when the intent calls for a voice the catalog lacks, and demand the POSTER faces when the intent is loud (an elegant serif on a cartel brief is a miscast, not restraint).
