...
it('opens a LOCAL-object dashboard on 30d but a CONNECTED one on the full range', function () {
    ...
    // A connected series → also defaults to 30d (the product default); the
    // reader pushes this window down into the source's start-date argument so
    // 30d re-fetches a real 30-day window.
    ...
    expect($connFilter['controls'][0]['default'])->toBe('30d')
        ->and(json_encode($connectedBuilt['page']))->toContain("default(params.range, '30d')")
        ->and(json_encode($connectedBuilt['page']))->not->toContain("default(params.range, '1y')");
});
