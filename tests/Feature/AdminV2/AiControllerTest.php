...
test('defaults tab renders with current settings + enabled models', function () {
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(fn ($page) => $page
            ->component('admin-v2/Ai/Defaults')
            ->has('defaults')
            ->has('chatModels.0')
...
test('catalog tab lists every catalog model', function () {
    ...
    $this->actingAs($admin)
        ->get('/admin2/ai/catalog')
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->component('admin-v2/Ai/Catalog')
...
test('updateDefaults saves the primary chat model', function () {
    ...
    $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['primaryChatModelId' => $model->id])
        ->assertRedirect();
...
test('updateDefaults rejects a non-chat model as primary', function () {
    ...
    $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['primaryChatModelId' => $embed->id])
...
test('updateDefaults clamps temperature to 0..2', function () {
    ...
    $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['temperature' => 5])
        ->assertSessionHasErrors(['temperature']);

    $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['temperature' => 0.3])
        ->assertRedirect();
});
...
test('embedding model swap dispatches RecomputeEmbeddingsJob per KB', function () {
    ...
    $response = $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['embeddingModelId' => $replacement->id]);
...
test('first-time embedding assignment does not dispatch the reindex job', function () {
    ...
    $this->actingAs($admin)
        ->patch('/admin2/ai/defaults', ['embeddingModelId' => $first->id])
        ->assertRedirect();
...
test('toggleModel flips is_enabled on the catalog row', function () {
    ...
    $this->actingAs($admin)
        ->patch("/admin2/ai/catalog/{$model->id}", ['enabled' => false])
        ->assertRedirect();
...
test('rotateKey writes the new api key to the provider and stamps rotated_at', function () {
    ...
    $this->actingAs($admin)
        ->post("/admin2/ai/providers/{$provider->id}/rotate-key", [
...
test('testConnection returns ok when a provider has credentials', function () {
    ...
    $this->actingAs($admin)
        ->postJson('/admin2/ai/test-connection', [
...
test('non-sysadmin is blocked from /admin2/ai', function () {
    $member = User::factory()->create();

    $this->actingAs($member)->get('/admin2/ai')->assertForbidden();
    $this->actingAs($member)->get('/admin2/ai/catalog')->assertForbidden();
    $this->actingAs($member)
        ->patch('/admin2/ai/defaults', ['temperature' => 0.2])
        ->assertForbidden();
});
