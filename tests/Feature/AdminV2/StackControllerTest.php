...
test('every stack item carries a status field', function () {
    ...
    $response = $this->actingAs($user)->get('/admin2/stack')->assertOk();
...
test('non-sysadmin is blocked from /admin2/stack', function () {
    $member = User::factory()->create();

    $this->actingAs($member)->get('/admin2/stack')->assertForbidden();
});
