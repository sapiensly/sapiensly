...
const streamSignature = computed(() =>
    props.messages.map((m) => `${m.id}:${m.content?.length ?? 0}:${m.status}`).join('|'),
);
                ...
                <div v-for="(m, i) in messages" :key="m.id">
                    ...
                    <div v-else class="group flex gap-4">
                        ...
                        <div class="min-w-0 flex-1 pt-1">
                            ...
                            >
                                <p>{{ m.error || m.content || t('chat.error_generic') }}</p>
                                <button
                                    v-if="isLast(i)"
                                    type="button"
                            ...
                            <template v-else>
                                ...
                                >
                                    ...
                                    {{ t('chat.tools.using', { tool: prettyToolName(toolActivity[m.id]) }) }}
                                </div>
                                <template v-for="(seg, si) in segmentsFor(m)" :key="si">
                                    <ArtifactCard
                                ...
                                <span
                                    v-if="m.status === 'pending' || m.status === 'streaming'"
                                ...
                                >
                                    <button
                                        ...
                                        @click="copy(m)"
                                    >
                                        <Check v-if="copiedId === m.id" class="size-3.5 text-sp-success" />
