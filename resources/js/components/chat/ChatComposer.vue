...
import type { ChatAttachmentDto, ChatModelOption, ChatToolOption } from '@/types/chatModule';
import axios from 'axios';
import { ArrowUp, ChevronDown, FileText, Globe, Loader2, Paperclip, Sparkles, Square, Wrench, X } from 'lucide-vue-next';
...
const emit = defineEmits<{
    submit: [payload: { content: string; attachmentIds: string[]; webSearch: boolean; toolIds: string[] }];
...
const selectedModel = computed(() => props.models.find((m) => m.value === props.model));
...
const canSubmit = computed(
    () => !props.busy && !uploading.value && (text.value.trim() !== '' || attachments.value.length > 0),
);
...
async function onFilesChosen(e: Event) {
    ...
    try {
        for (const file of files) {
            ...
            const { data } = await axios.post(`/chat/${props.chatId}/attachments`, form);
...
<template>
    <div>
        <div class="rounded-[1.625rem] border border-medium bg-surface px-2 py-2 shadow-md transition-all focus-within:border-strong focus-within:shadow-lg">
            ...
            <div v-if="attachments.length" class="flex flex-wrap gap-2 px-2 pb-1 pt-1.5">
                <span
                    ...
                    :key="a.id"
                    class="inline-flex items-center gap-1.5 rounded-lg border border-medium bg-white/5 py-1 pl-1.5 pr-1 text-xs text-ink"
                >
                    <FileText class="size-3.5 text-ink-subtle" />
                    <span class="max-w-[160px] truncate">{{ a.original_name }}</span>
                    <button type="button" class="rounded p-0.5 text-ink-subtle hover:bg-white/10 hover:text-ink" @click="removeAttachment(a.id)">
                        <X class="size-3" />
                    </button>
                </span>
            </div>
            ...
            <textarea
                ...
                rows="1"
                class="block max-h-80 w-full resize-none bg-transparent px-3 pb-1 pt-2.5 text-[15px] leading-6 text-ink placeholder:text-ink-subtle focus:outline-none"
                :autofocus="autofocus"
                @input="autoGrow"
                @keydown="onKeydown"
            />

            <p v-if="uploadError" class="px-3 pb-1 text-[11px] text-sp-danger">{{ uploadError }}</p>
            ...
            <div class="flex items-center justify-between gap-2 px-1 pt-0.5">
                <div class="flex items-center gap-0.5">
                    <button
                        ...
                        @click="pickFiles"
                    >
                        <Loader2 v-if="uploading" class="size-[18px] animate-spin" />
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            ...
                            >
                                ...
                                <span class="max-w-[200px] truncate">{{ selectedModel?.label ?? t('chat.model_picker.label') }}</span>
                        ...
                        <DropdownMenuContent align="start" class="max-h-80 w-64 overflow-y-auto">
                            <DropdownMenuRadioGroup
                                :model-value="model ?? ''"
                                @update:model-value="(v) => emit('update:model', v as string)"
                            >
                                <template v-for="[provider, items] in grouped" :key="provider">
                                    <DropdownMenuLabel class="text-[10px] uppercase tracking-wider text-ink-subtle">
                                        {{ provider }}
                                    </DropdownMenuLabel>
                                    <DropdownMenuRadioItem
                                        v-for="m in items"
                    ...
                    <button
                        ...
                        :class="[
                            ...
                            webSearch ? 'bg-accent-blue/15 text-accent-blue' : 'text-ink-muted hover:bg-white/10 hover:text-ink',
                        ]"
                        @click="webSearch = !webSearch"
                    >
                        <Globe class="size-3.5" />
                        <span class="hidden sm:inline">{{ t('chat.web_search') }}</span>
                    </button>
                    ...
                    <DropdownMenu v-if="tools.length">
                        <DropdownMenuTrigger as-child>
                            <button
                                type="button"
                                :title="t('chat.tools.label')"
                                :class="[
                                    'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[13px] font-medium transition-colors',
                                    toolIds.length ? 'bg-accent-blue/15 text-accent-blue' : 'text-ink-muted hover:bg-white/10 hover:text-ink',
                                ]"
                            >
                                <Wrench class="size-3.5" />
                                <span class="hidden sm:inline">{{ t('chat.tools.label') }}</span>
                                <span
                                    v-if="toolIds.length"
                        ...
                        <DropdownMenuContent align="start" class="max-h-80 w-64 overflow-y-auto">
                            ...
                            >
                                ...
                                <span class="ml-auto text-[10px] uppercase text-ink-subtle">{{ tool.type }}</span>
                            </DropdownMenuCheckboxItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
        ...
        <p class="mt-2 text-center text-[11px] text-ink-subtle">{{ t('chat.disclaimer') }}</p>
    </div>
</template>
