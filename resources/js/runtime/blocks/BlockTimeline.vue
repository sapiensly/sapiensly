    ...
    return rows
        .map<Entry | null>((r) => {
            ...
            if (colorField.value?.type === 'single_select' && cSlug) {
                const opt = colorField.value.options?.find((o) => o.value === r.data[cSlug]);
            ...
            return {
                ...
                date: valid ? d : null,
                dateLabel: valid
                    ? d!.toLocaleString(props.locale, { dateStyle: 'medium', timeStyle: 'short' })
                    : '—',
                title: tSlug ? String(r.data[tSlug] ?? r.id) : r.id,
                description: sSlug ? (r.data[sSlug] ? String(r.data[sSlug]) : null) : null,
                color,
            };
        })
...
<template>
    <div :class="['rounded-sp-sm border p-5', t.surface]">
        ...
        <p v-if="entries.length === 0" :class="['text-center text-xs', t.textMuted]">No entries.</p>
        <ol v-else class="relative ml-3 space-y-4 border-l border-soft pl-5">
            <li v-for="e in entries" :key="e.id" class="relative">
                <span
                    class="absolute -left-[26px] top-1 size-2.5 rounded-pill border-2"
                    :style="{ background: e.color, borderColor: 'var(--sp-navy, #0b1530)' }"
                />
                <p :class="['text-[11px]', t.textSubtle]">{{ e.dateLabel }}</p>
                <p :class="['mt-0.5 text-sm font-medium', t.text]">{{ e.title }}</p>
                <p v-if="e.description" :class="['mt-0.5 text-xs', t.textMuted]">{{ e.description }}</p>
            </li>
        </ol>
    </div>
</template>
