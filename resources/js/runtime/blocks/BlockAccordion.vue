...
const open = ref<Record<string, boolean>>(
    Object.fromEntries(
        props.block.sections.map((s) => [s.id, !!s.default_open]),
    ),
);
...
<template>
    <div
        :class="[
            'divide-y divide-soft overflow-hidden rounded-sp-sm border',
            t.surface,
        ]"
    >
        <section v-for="section in block.sections" :key="section.id">
            <button
                ...
                :class="[
                    'flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-surface',
                    t.text,
                ]"
            ...
            <div
                ...
                class="space-y-3 border-t border-soft px-4 py-4"
            >
                <AppRenderer
