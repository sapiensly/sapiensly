...
const field = computed<FieldDef | undefined>(() =>
    resolveField(props.objects.find((o) => o.id === props.block.data_source.object_id), props.block.field_id),
);
...
const words = computed(() => {
    ...
    for (const r of props.data?.rows ?? []) {
        ...
        for (const v of vals) {
            ...
            const key = field.value?.type === 'single_select' || field.value?.type === 'multi_select'
                ? (field.value.options?.find((o) => o.value === v)?.label ?? String(v))
                : String(v);
...
<template>
    <div :class="['rounded-sp-sm border p-5', t.surface]">
        ...
        <p v-if="words.length === 0" :class="['py-6 text-center text-xs', t.textMuted]">No data.</p>
        <div v-else class="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
            <span
                v-for="w in words"
                :key="w.word"
                class="leading-tight"
                :style="{ fontSize: w.size + 'rem', fontWeight: w.weight, color: w.color }"
