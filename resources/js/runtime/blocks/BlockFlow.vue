...
<template>
    <div class="flex flex-col gap-3">
        <p v-if="block.label" class="text-[11px] font-semibold uppercase tracking-wider" :style="{ opacity: 0.6 }">{{ block.label }}</p>
        <div :class="['flex', isRow ? 'flex-col items-stretch gap-3 md:flex-row md:items-stretch' : 'flex-col gap-3']">
            <template v-for="(step, i) in block.steps" :key="step.id ?? i">
                ...
                >
                    <div class="flex items-center gap-2">
                        <span v-if="step.icon" class="text-lg leading-none">{{ step.icon }}</span>
                        <span
                    ...
                    <p v-if="step.description" class="text-xs" :style="{ opacity: 0.7 }">{{ step.description }}</p>
                </div>
                <div
