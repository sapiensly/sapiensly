...
const systemLimit = computed<number | null>(() => {
    const candidates = [
        props.budget?.system_monthly_budget,
        props.budget?.platform_system_cap,
...
const systemSpentThisBudgetPeriod = computed(
    () => props.budget?.period_to_date.system ?? 0,
);
const systemUsagePct = computed(() =>
    systemLimit.value
        ? Math.min(
              100,
              Math.round(
                  (systemSpentThisBudgetPeriod.value / systemLimit.value) * 100,
              ),
          )
        : null,
);
...
const combinedSeries = computed(() =>
    props.report.series.own.map(
        (v, i) =>
            Math.round((v + props.report.series.system[i]) * 1_000_000) /
            1_000_000,
    ),
);
...
const chartSeries = computed(() => [
    {
        ...
        points: props.report.series.system,
    },
    {
        label: 'Own (BYOK)',
        tint: 'var(--sp-spectrum-magenta)',
        points: props.report.series.own,
    },
]);

const scopeLabel = computed(() =>
    props.scope.type === 'organization'
        ? `Organization · ${props.scope.name}`
        : `Personal · ${props.scope.name}`,
);
...
const seriesHeading = computed(() =>
    props.period.granularity === 'hour' ? 'Hourly spend' : 'Daily spend',
);
...
const chartTooltipLabels = computed(() =>
    props.report.series.labels.map((label) => {
        const day = new Date(
            `${label.slice(0, 10)}T00:00:00`,
        ).toLocaleDateString(undefined, {
            day: 'numeric',
            month: 'short',
        });
        return props.period.granularity === 'hour'
            ? `${day}, ${label.slice(11)}`
            : day;
    }),
);
...
function longDate(iso: string): string {
    const sameYear =
        props.period.from.slice(0, 4) === props.period.to.slice(0, 4);
    return new Date(`${iso}T00:00:00`).toLocaleDateString(undefined, {
        day: 'numeric',
        month: 'short',
...
<template>
    ...
    <AppLayoutV2 title="AI Spend">
        ...
        <div class="flex flex-col gap-6 px-0 py-4 sm:p-6">
            ...
            <PageHeader title="AI Spend" :description="scopeLabel">
                <template #actions>
                    ...
                    <div class="flex flex-col items-end gap-1">
                        <div class="-mx-1 max-w-full overflow-x-auto px-1">
                            ...
                            >
                                <Link
                                    ...
                                    :title="p.label"
                                    :class="[
                                        'rounded-pill px-3 py-1 text-xs whitespace-nowrap transition-colors',
                                        p.key === period.key
                        ...
                        <p
                            class="px-1 text-right text-[11px] text-ink-subtle tabular-nums"
                        >
                            {{ rangeCaption }}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center gap-4 text-xs text-ink-muted">
                        <span class="inline-flex items-center gap-1.5">
                            <span
                                ...
                                style="background: var(--sp-accent-blue)"
                            />
                            System
                        </span>
                        <span class="inline-flex items-center gap-1.5">
                            <span
                                class="size-2 rounded-full"
                                style="background: var(--sp-spectrum-magenta)"
                            />
                            Own
                        </span>
                    </div>
            ...
            <section
                v-if="scope.type === 'organization'"
                class="rounded-sp-sm border border-soft bg-navy p-5"
            >
                <h2 class="mb-1 text-sm font-medium text-ink">
                    Monthly budget
                </h2>
                <p class="mb-4 text-xs text-ink-muted">
                    Cap your AI spend. System models (paid by the platform) are
                    also bound by any platform ceiling; own (BYOK) models are
                    capped only if you set a limit.
                </p>

                <div v-if="systemLimit !== null && budget" class="mb-4">
                    <div
                        class="mb-1 flex justify-between gap-3 text-xs text-ink-muted"
                    >
                        <span
                            >System spend since
                            {{ shortDate(budget.period_to_date.since) }}</span
                        >
                        <span class="whitespace-nowrap">
                            {{ money(systemSpentThisBudgetPeriod) }} /
                            {{ money(systemLimit) }} ({{ systemUsagePct }}%)
                        </span>
                    </div>
                    <div class="h-2 overflow-hidden rounded-full bg-surface">
                        <div
                            ...
                            :class="
                                (systemUsagePct ?? 0) >= 100
                                    ? 'bg-sp-danger'
                                    : (systemUsagePct ?? 0) >=
                                        budgetForm.alert_threshold_pct
                                      ? 'bg-sp-warning'
                                      : 'bg-accent-blue'
                            "
                    ...
                    <p
                        v-if="budget?.platform_system_cap != null"
                        class="mt-1 text-[10px] text-ink-subtle"
                    >
                        Platform ceiling:
                        {{ money(budget.platform_system_cap) }}
                    </p>
                </div>
                ...
                >
                    ...
                    <label
                        class="flex items-center gap-2 self-end text-xs text-ink"
                    >
                        <input
                            v-model="budgetForm.enforcement_enabled"
                            type="checkbox"
                            class="rounded border-medium"
                        />
            ...
            <section class="rounded-sp-sm border border-soft bg-navy p-5">
                ...
                <p
                    v-if="report.by_service.length === 0"
                ...
                <!-- by service → models -->
                <div
                    v-else-if="serviceBreakdown === 'models'"
            ...
            <!-- Top models -->
            <section class="rounded-sp-sm border border-soft bg-navy p-5">
                <h2 class="mb-3 text-sm font-medium text-ink">
                    Top models by cost
                </h2>
                <p
                    v-if="report.by_model.length === 0"
                    class="text-xs text-ink-muted"
                >
                    No AI usage recorded in this period yet.
                </p>
