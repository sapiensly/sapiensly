...
const currentModel = ref<string | null>(
    props.activeChat?.model ?? props.defaultModel ?? props.models[0]?.value ?? null,
);
...
function upsert(message: ChatMessageDto) {
    ...
    else messages.value = messages.value.map((m) => (m.id === message.id ? { ...m, ...message } : m));
}
...
function subscribe(id: string) {
    ...
    channel.listen('.ChatStreamChunk', (data: { message_id: string; delta: string }) => {
        if (stopped.value.has(data.message_id)) return;
        const existing = messages.value.find((m) => m.id === data.message_id);
        if (existing) {
    ...
    channel.listen('.ChatToolCall', (data: { message_id: string; tool_name: string }) => {
        toolActivity.value = { ...toolActivity.value, [data.message_id]: data.tool_name };
    });

    channel.listen('.ChatStreamComplete', (payload: { message: ChatMessageDto }) => {
    ...
    channel.listen('.ChatStreamError', (data: { message_id: string; error: string }) => {
        messages.value = messages.value.map((m) =>
            m.id === data.message_id ? { ...m, status: 'error', error: data.error } : m,
        );
...
watch(
    ...
    (id) => {
        ...
        currentModel.value = props.activeChat?.model ?? currentModel.value;
...
// ----- Sending -----
async function send(payload: { content: string; attachmentIds: string[]; webSearch?: boolean; toolIds?: string[] }) {
    const { content, attachmentIds, webSearch = false, toolIds = selectedToolIds.value } = payload;

    if (!activeId.value) {
        // No chat yet — create it with the first turn in one navigation.
        router.post('/chat', { content, model: currentModel.value, web_search: webSearch, tool_ids: toolIds });
        return;
    }
    ...
    try {
        ...
        messages.value = messages.value.map((m) => (m.id === userMsg.id ? data.user_message : m));
        upsert(data.placeholder);
    } catch {
        messages.value = messages.value.map((m) =>
            m.id === userMsg.id ? { ...m, status: 'error', error: t('chat.send_failed') } : m,
        );
    }
}
...
function stop() {
    const streaming = messages.value.find((m) => m.status === 'pending' || m.status === 'streaming');
    if (!streaming) return;
    ...
    if (activeId.value && !streaming.id.startsWith('temp-')) {
        axios.post(`/chat/${activeId.value}/stop`, { message_id: streaming.id }).catch(() => {});
    }
    ...
    >
        <div class="flex min-h-0 flex-1">
            ...
            >
                ...
                >
                    <ArtifactPanel :artifact="activeArtifact" @close="closeArtifact" />
                </div>
            </Transition>
        </div>
    </AppLayoutV2>
