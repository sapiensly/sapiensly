...
interface Message {
    ...
    status: 'pending' | 'applied' | 'rejected' | 'reverted' | 'none' | 'streaming';
...
interface SchemaPayload {
    objects: Array<ObjectDef & { system_fields?: Array<{ id: string; slug: string; name: string; type: string }> }>;
    record_counts: Record<string, number>;
    workflows_by_object: Record<string, Array<{ id: string; name: string; trigger_type: string | null }>>;
}
...
const resizable = computed(() => fullscreenPanel.value === null && isLargeScreen.value);
...
const gridClass = computed(() => {
    ...
    return isLargeScreen.value ? 'grid min-h-0 flex-1 grid-cols-1' : 'grid min-h-0 flex-1 grid-cols-1 gap-4';
});
...
const gridStyle = computed(() => {
    ...
    return { gridTemplateColumns: `${chatWidth.value}px ${RESIZER_WIDTH}px minmax(0, 1fr)` };
});
...
function clampChatWidth(width: number): number {
    ...
    const maxChat = Math.max(MIN_CHAT_WIDTH, containerWidth - MIN_WORK_WIDTH - RESIZER_WIDTH);
...
function fullscreenClassFor(panel: 'chat' | 'work'): string[] {
    ...
    return ['fixed', 'inset-0', 'z-50', '!rounded-none', '!border-0', 'h-screen', 'w-screen'];
}
...
const slashMatches = computed<SlashCommand[]>(() =>
    slashFilterText.value === null ? [] : matchingCommands(slashFilterText.value),
);
...
function moveSlashHighlight(delta: number) {
    ...
    if (len === 0) return;
    slashHighlightedIndex.value = (slashHighlightedIndex.value + delta + len) % len;
}
 ...
 */
function onWireframeImported(payload: { messages: unknown[]; latest_message_id?: string }) {
...
function detectChipLocale(text: string): ChipLocale | null {
    ...
    const esWords = new Set([
        'el', 'la', 'los', 'las', 'un', 'una', 'de', 'que', 'y', 'es', 'para',
        'con', 'por', 'qué', 'cómo', 'cuándo', 'dónde', 'hola', 'gracias',
        'o', 'en', 'no', 'sí', 'este', 'esta', 'ese', 'esa', 'quiero',
        'agregar', 'tengo', 'tienes', 'puedo', 'soy', 'pero', 'también',
    ]);
    const enWords = new Set([
        'the', 'and', 'or', 'is', 'are', 'to', 'of', 'a', 'an', 'that',
        'what', 'how', 'when', 'where', 'hi', 'hello', 'thanks', 'i',
        'you', 'my', 'your', 'want', 'add', 'have', 'has', 'can', 'with',
        'for', 'on', 'in', 'this', 'these', 'them', 'but', 'also',
    ]);
...
function tInConversationLocale(fullKey: string): string {
    const dict = i18nMessages.value[conversationLocale.value] as Record<string, unknown> | undefined;
...
const chipSuggestions = computed<ChipSuggestion[]>(() => {
    ...
    if (!hasObjects && messages.value.length === 0) {
        return [
            chip('app-from-scratch', 'apps.builder.chip.app_from_scratch', Sparkles),
            chip('first-object', 'apps.builder.chip.first_object', Database),
            chip('suggest-objects', 'apps.builder.chip.suggest_objects', Lightbulb),
        ];
    }
    ...
    if (viewMode.value === 'schema') {
        ...
        return [
            ...
            chip('add-validations', 'apps.builder.chip.add_validations', ListChecks),
    ...
    if (viewMode.value === 'manifest') {
        return [
            chip('explain-manifest', 'apps.builder.chip.explain_manifest', FileText),
    ...
    if (!hasPages || !hasBlocksOnCurrentPage) {
        return [
            chip('empty-table', 'apps.builder.chip.empty_page.add_table', Database),
            chip('empty-dashboard', 'apps.builder.chip.empty_page.add_dashboard', LayoutDashboard),
    ...
    return [
        ...
        chip('add-action-column', 'apps.builder.chip.add_action_column', MousePointerClick),
...
const currentViewMeta = computed(() => {
    const map = {
        ...
        workflows: { icon: WorkflowIcon, heading: t('apps.builder.workflows_heading') },
...
async function send() {
    ...
    if (text.startsWith('/')) {
        ...
        if (parsed !== null) {
            ...
            if (expanded !== null) {
                ...
                errorText.value = tInConversationLocale(`apps.builder.slash.${parsed.command.id}.usage`)
    ...
    const messageText = text || (stagedFile ? t('apps.builder.attachment_default_message') : '');
    input.value = '';
...
async function requestVisualReview() {
    ...
    if (!node) {
        errorText.value = 'No hay preview que capturar — cambia al tab Preview primero.';
        return;
    }
    ...
    try {
        ...
        const MAX_DIM = 1600;
        const scaleDown = Math.min(1, MAX_DIM / Math.max(rawCanvas.width, rawCanvas.height));
...
function subscribe() {
    ...
    channel.listen(
        ...
        (payload: { message: Message }) => {
            ...
            router.reload({ only: ['preview', 'manifest'], preserveScroll: true });
        },
    );
...
function startNewConversation() {
    if (messages.value.length > 0 && ! window.confirm(t('apps.builder.new_conversation_confirm'))) {
        return;
    }
    router.post(
        ...
        <div class="flex min-h-0 flex-1 flex-col gap-4 px-7 py-5">
            <header class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-[18px] font-semibold leading-tight text-ink">
                            <Sparkles class="-mt-0.5 mr-1 inline size-4 text-accent-blue" />
                ...
                <div class="inline-flex items-center rounded-pill border border-medium bg-surface p-0.5">
                    <button
                        v-for="m in ([
                            ...
                            { id: 'workflows', label: t('apps.builder.tab_workflows'), icon: WorkflowIcon },
            ...
            <div ref="gridEl" :class="gridClass" :style="gridStyle">
                ...
                >
                    ...
                    >
                        <p class="rounded-pill bg-navy/90 px-4 py-2 text-sm font-medium text-accent-blue">
                            <Paperclip class="mr-1 inline size-4" />
                            {{ t('apps.builder.drop_to_attach') }}
                        </p>
                    </div>

                    <header class="flex items-center justify-between gap-2 border-b border-soft px-4 py-3">
                            ...
                            >
                                <div class="flex items-center justify-between gap-3 px-3 py-2">
                                    <div class="flex items-center gap-2 min-w-0">
                                        <span
                                            :class="[
                                                'inline-flex items-center rounded-pill border px-2 py-0.5 text-[10px] uppercase tracking-wider',
                                                statusTone(m.status),
                                            ]"
                                        >
                                            <Check v-if="m.status === 'applied'" class="size-3 mr-0.5" />
                                    ...
                                    <div class="flex shrink-0 items-center gap-1.5">
                                        ...
                                        >
                                            <ChevronDown
                                                ...
                                                :class="openPatches[m.id] ? 'rotate-180' : ''"
                                            />
                                            {{ openPatches[m.id] ? t('apps.builder.hide_patch') : t('apps.builder.show_patch') }}
                                        </button>
                                    </div>
                                </div>
                                <pre
                    ...
                    <footer class="border-t border-soft px-4 py-3">
                        ...
                        >
                            ...
                            />
                            <span class="flex-1 truncate text-[11px] text-ink-muted">
                                {{ attachedFile?.name }}
                            </span>
                            <button
                                type="button"
                ...
                >
                    ...
                    <GripVertical
                        ...
                        :class="isResizing ? 'text-accent-blue' : 'text-ink-muted group-hover:text-accent-blue group-focus-visible:text-accent-blue'"
                    />
                </div>
                ...
                >
                    <header class="flex items-center justify-between gap-3 border-b border-soft px-4 py-3">
                        <div class="flex items-center gap-2">
                            <component :is="currentViewMeta.icon" class="size-3.5 text-ink-muted" />
                            <h2 class="text-xs font-medium uppercase tracking-wide text-ink-muted">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <nav v-if="preview && preview.pages.length > 1" class="flex flex-wrap gap-1">
                                <button
                    ...
                    <SchemaView v-else-if="viewMode === 'schema'" :schema="schema" :app-id="app.id" class="flex-1 min-h-0" />

                    <AppWorkflowsTab
                        v-else-if="viewMode === 'workflows'"
                        :app-id="app.id"
                        :workflows="(manifest?.workflows as never[] | undefined) ?? []"
                        :objects="(manifest?.objects as never[] | undefined) ?? []"
                        @manifest-updated="() => router.reload({ only: ['preview', 'manifest', 'schema'], preserveScroll: true })"
                    ...
                    >{{ manifest ? JSON.stringify(manifest, null, 2) : t('apps.show.no_manifest') }}</pre>
                </section>
            </div>
        </div>
 ...
 * bubble.
 */
.builder-md :deep(p) { margin: 0.3rem 0; line-height: 1.55; }
.builder-md :deep(p:first-child) { margin-top: 0; }
.builder-md :deep(p:last-child) { margin-bottom: 0; }
.builder-md :deep(h1) { font-size: 1.05rem; font-weight: 600; margin: 0.6rem 0 0.3rem; }
.builder-md :deep(h2) { font-size: 1rem; font-weight: 600; margin: 0.55rem 0 0.25rem; }
.builder-md :deep(h3) { font-size: 0.95rem; font-weight: 600; margin: 0.5rem 0 0.2rem; }
.builder-md :deep(ul), .builder-md :deep(ol) { padding-left: 1.1rem; margin: 0.3rem 0; }
.builder-md :deep(ul) { list-style: disc; }
.builder-md :deep(ol) { list-style: decimal; }
.builder-md :deep(li) { margin: 0.1rem 0; }
.builder-md :deep(li > p) { margin: 0; }
.builder-md :deep(strong) { font-weight: 600; }
.builder-md :deep(em) { font-style: italic; }
...
.builder-md :deep(pre code) { background: transparent; padding: 0; }
...
.builder-md :deep(th), .builder-md :deep(td) {
    border: 1px solid rgba(255, 255, 255, 0.1);
    padding: 0.2rem 0.45rem;
    text-align: left;
}
.builder-md :deep(th) { font-weight: 600; background: rgba(255, 255, 255, 0.04); }
</style>
