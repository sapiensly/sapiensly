...
import InviteUserDialog from '@/components/admin-v2/InviteUserDialog.vue';
import UserDetailSheet from '@/components/admin-v2/UserDetailSheet.vue';
import UserTable from '@/components/admin-v2/UserTable.vue';
...
<template>
    <Head :title="t('admin_v2.nav.users')" />

    <AdminV2Layout :title="t('admin_v2.nav.users')">
        <div class="space-y-5">
            <!-- Header -->
            <header class="flex items-start justify-between gap-4">
                <div class="space-y-1">
                    <h1 class="text-[22px] font-semibold leading-tight text-ink">
                        {{ t('admin_v2.users.heading') }}
                    </h1>
                    <p class="text-xs text-ink-muted">
                        {{
                            t('admin_v2.users.description_count', {
            ...
            <div class="space-y-3">
                <div class="relative">
                    ...
                    <Input
                        v-model="q"
                        :placeholder="t('admin_v2.users.filter.search')"
                ...
                <Select v-model="role">
                    ...
                    <SelectContent>
                        <SelectItem value="any">
                            {{ t('admin_v2.users.filter.role_any') }}
                        </SelectItem>
                        <SelectItem value="sysadmin">
                            {{ t('admin_v2.users.role.sysadmin') }}
                        </SelectItem>
                ...
                <Select v-model="status">
                    ...
                    <SelectContent>
                        <SelectItem value="any">
                            {{ t('admin_v2.users.filter.status_any') }}
                        </SelectItem>
                        <SelectItem value="active">
                            {{ t('admin_v2.users.status.active') }}
                        </SelectItem>
                        <SelectItem value="unverified">
                            {{ t('admin_v2.users.status.unverified') }}
                        </SelectItem>
                        <SelectItem value="blocked">
                            {{ t('admin_v2.users.status.blocked') }}
                        </SelectItem>
                    </SelectContent>
                </Select>
            </div>
            ...
            >
                <span class="text-ink-muted">
                    {{
                        t('admin_v2.users.selection', { count: selectionCount })
                    }}
                </span>
                <button
                    type="button"
                ...
                >
                    {{ t('admin_v2.users.clear_selection') }}
                </button>
            </div>
            ...
            >
                <span>
                    {{
                        t('admin_v2.users.showing_count', {
                ...
                <div class="flex items-center gap-4">
                    <button
                        ...
                        "
                    >
                        {{ t('admin_v2.users.pagination_prev') }}
                    </button>
                    <span class="text-ink-subtle">
                        {{
                            t('admin_v2.users.page_of', {
                    ...
                    <button
                        ...
                        "
                    >
                        {{ t('admin_v2.users.pagination_next') }}
                    </button>
                </div>
            </nav>
        </div>
