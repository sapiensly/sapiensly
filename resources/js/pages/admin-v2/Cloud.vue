...
import CapacityBar from '@/components/admin-v2/CapacityBar.vue';
...
<template>
    <Head :title="t('admin_v2.nav.cloud')" />

    <AdminV2Layout :title="t('admin_v2.nav.cloud')">
        <div class="space-y-6">
            <header class="space-y-1">
                <h1 class="text-[22px] font-semibold leading-tight text-ink">
                    {{ t('admin_v2.cloud.heading') }}
                </h1>
                <p class="text-xs text-ink-muted">
                    {{ t('admin_v2.cloud.description') }}
                </p>
            </header>
            ...
            <div class="grid gap-4 xl:grid-cols-2">
                <SettingsCard
                    :icon="HardDrive"
                    :title="t('admin_v2.cloud.storage.title')"
                    :description="t('admin_v2.cloud.storage.description')"
                >
                    <template v-if="storage">
                        <dl class="space-y-2 text-sm">
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.storage.driver') }}
                                </dt>
                                <dd
                            ...
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.storage.bucket') }}
                                </dt>
                            ...
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.storage.region') }}
                                </dt>
                                <dd class="font-mono text-xs text-ink">
                                    {{ storage.region }}
                                </dd>
                        ...
                        <div class="pt-1">
                            <p class="mb-1.5 text-[10px] tracking-wider text-ink-subtle uppercase">
                                {{ t('admin_v2.cloud.storage.capacity') }}
                            </p>
                            <CapacityBar
                            ...
                            >
                                {{ t('admin_v2.cloud.storage.capacity_hint') }}
                            </p>
                        </div>
                    </template>
                    <p
                    ...
                    >
                        {{ t('admin_v2.cloud.storage.empty') }}
                    </p>
                </SettingsCard>

                <SettingsCard
                    :icon="Database"
                    :title="t('admin_v2.cloud.database.title')"
                    :description="t('admin_v2.cloud.database.description')"
                ...
                >
                    <template v-if="database">
                        <dl class="space-y-2 text-sm">
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.database.engine') }}
                                </dt>
                                <dd class="font-mono text-xs text-ink">
                            ...
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.database.host') }}
                                </dt>
                            ...
                            <div class="flex items-center justify-between">
                                <dt class="text-xs text-ink-muted">
                                    {{ t('admin_v2.cloud.database.size') }}
                                </dt>
                                <dd class="font-mono text-xs text-ink">
                        ...
                        <div class="pt-1">
                            <div class="mb-1.5 flex items-center justify-between">
                                ...
                                >
                                    {{ t('admin_v2.cloud.database.connections') }}
                                </p>
                    ...
                    <p
                        ...
                        class="rounded-xs border border-dashed border-soft p-3 text-xs text-ink-muted"
                    >
                        {{ t('admin_v2.cloud.database.empty') }}
                    </p>
                </SettingsCard>
            </div>
            ...
            <SettingsCard
                :icon="Cpu"
                :title="t('admin_v2.cloud.pgvector.title')"
                :description="t('admin_v2.cloud.pgvector.description')"
            >
                <template v-if="pgvector.enabled">
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        >
                            <p
                                class="text-[10px] tracking-wider text-ink-subtle uppercase"
                            >
                                {{ t('admin_v2.cloud.pgvector.indexes') }}
                            </p>
                        ...
                        >
                            <p
                                class="text-[10px] tracking-wider text-ink-subtle uppercase"
                            >
                                {{ t('admin_v2.cloud.pgvector.vectors') }}
                            </p>
                            <p class="font-mono text-lg font-semibold text-ink">
                        ...
                        >
                            <p
                                class="text-[10px] tracking-wider text-ink-subtle uppercase"
                            >
                                {{ t('admin_v2.cloud.pgvector.size') }}
                            </p>
                            <p class="font-mono text-lg font-semibold text-ink">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-ink-muted">
                            {{ t('admin_v2.cloud.pgvector.enabled_label') }}
                        </span>
                    </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow class="border-soft hover:bg-transparent">
                                    ...
                                    >
                                        {{ t('admin_v2.cloud.pgvector.col.name') }}
                                    </TableHead>
                                    <TableHead
                                        class="text-[10px] font-semibold tracking-wider text-ink-subtle uppercase"
                                    >
                                        {{ t('admin_v2.cloud.pgvector.col.table') }}
                                    </TableHead>
                                    <TableHead
                                        class="text-[10px] font-semibold tracking-wider text-ink-subtle uppercase"
                                    >
                                        {{ t('admin_v2.cloud.pgvector.col.metric') }}
                                    </TableHead>
                                    <TableHead
                                        class="text-right text-[10px] font-semibold tracking-wider text-ink-subtle uppercase"
                                    >
                                        {{ t('admin_v2.cloud.pgvector.col.rows') }}
                                    </TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                <TableEmpty
                                    ...
                                    :colspan="4"
                                >
                                    {{ t('admin_v2.cloud.pgvector.no_indexes') }}
                                </TableEmpty>
                                <TableRow
                ...
                >
                    {{ t('admin_v2.cloud.pgvector.disabled') }}
                </div>
            </SettingsCard>
        </div>
    </AdminV2Layout>
