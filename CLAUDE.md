...
**Design lint** — non-blocking `design_smell` warnings from `ManifestValidator`, returned by `propose_change` alongside a message telling the model not to report success until each is fixed or explained. They are warnings, not errors, so a pre-existing one never makes an app unsaveable. Each came from a real build: R1 a page of nothing but headings/spacers · R3 a block needing `{{params.X}}` nothing provides · R4 two fields of one object wearing the same name · R5 a total built by summing unit prices · R6 a board grouped by a select nothing fills in · R11 a capture typed flat (signature/photo as text, lat+lng as a `number` pair — matched through `SemanticLexicon`, so the app's locale decides the vocabulary) · R12 a `navigate` whose target no page serves · R13 an object with a list block and no `record_detail`, which fires only when the app already uses detail pages for something else (none at all is a design choice; four out of six is an oversight) · R14 a second OVERVIEW page — aggregate blocks, nothing that touches one record — whose objects are a SUBSET of an earlier overview's, i.e. it charts nothing the first already charts · **R15** `{{current_user.id}}` filling a RELATION, in a form default or an action value — that is the id of the platform USER row, never a record id, so every submit is refused with «No X record matches '1'» · **R16** a `create_record` whose `values` omits a required field with no `default` — `values` is the WHOLE payload (`AppActionExecutor` resolves that map and nothing merges the form into it), so the form can never be filed, and rendering the field on the form does NOT fill it · **R17** an action overwriting a field the same form renders, which makes the control a lie: whatever the person chose is discarded (readonly and `visible_if` fields are exempt — those are legitimately the app's to supply).

R15–R17 all came off ONE live page, which is the argument for measuring a rail by what it prevents rather than by how odd it looks: a check-in form wrote `{{current_user.id}}` into «Empleado» (R15) while its `create_record` never wrote the required «Ubicación» (R16) and overrode both visible controls (R17), so the page could not be used at all and each fix only uncovered the next. R16 also caught the platform doing it to itself — `AppScaffolder`'s POS «nueva comanda» button set the status and not the required folio, so the till's first tap failed on a page the scaffolder had just built; `posNewOrderValues` now seeds every required field it can answer honestly and builds NO POS screen when it cannot. Three tool descriptions and the manifest schema said «use {{current_user.id}}» with no mention of relations; that is where the defect was learned. R12 and R13 walk the block tree GENERICALLY: descending only `blocks` misses everything inside `tabs[i].blocks`, which is where the central object's list lives on every scaffolded app — the first cut of R13 did exactly that, and a by-hand review of the same manifest missed the same link. `ScaffolderQualityTest` asserts a freshly scaffolded app raises zero design warnings; keep it that way.
...
**Tracking is the one thing here that is OFF by default** (`settings.tracking`, read only by `App\Support\Tracking\TrackingPolicy`) — the exact reverse of `settings.offline`, and for the reverse reason: an app that stops working in a basement is broken, an app that follows people who did not agree is an incident. **The load-bearing fact is that the web has no background geolocation.** `watchPosition` dies when the phone locks, and no service worker gets it back — so a trail exists only while the app is open, a silence is stored AS a gap (`considerFix`, `tracking.ts`) rather than papered over, and the bar says so in words. A straight line drawn across a hole reads as a journey nobody made. Consent is structural rather than a checkbox: nothing records until somebody taps start on a `tracking` block, a non-dismissible bar runs the whole time, stop is immediate, and a stopped session is refused server-side (`409`) so a stuck client cannot override it. The block is per PAGE because *which* page is the whole judgement — the visit being driven to, never a dashboard.

**The geofence is the reason to turn it on at all**; the breadcrumb trail is the part that feels like surveillance. `location.arrived` / `location.departed` fire through the ordinary `WorkflowTriggerDispatcher` with a `record` payload, so filters and every existing step work unchanged. Four rails in `Geofence` + `TrackingService`, each with a failure behind it: **hysteresis** (leaving is judged against a wider ring than arriving, because a stationary phone wanders tens of metres and a single radius fires arrive/depart forty times for one visit — forty messages to a customer); **`canDecide`** (a fix vaguer than the fence decides nothing and leaves the state *null*, rather than writing "outside" for a phone that is merely unsure — the first cut got this wrong and the next good fix read as an arrival that never happened); **no event for the state a session STARTED in** (opening the app at the customer's door is not arriving, and firing there sends "vamos en camino" from their doorstep); and **the target is copied onto the session at start**, so moving a record's pin mid-visit cannot retroactively change where somebody was judged to have arrived. `retain_days` is a promise enforced by `tracking:prune`, which also closes sessions a dead phone abandoned — "who is being followed right now" has to stay answerable. Tests: `tests/Feature/Apps/LocationTrackingTest.php`, `tests/Unit/Support/Tracking/GeofenceTest.php`, `resources/js/runtime/tracking.test.ts`.
